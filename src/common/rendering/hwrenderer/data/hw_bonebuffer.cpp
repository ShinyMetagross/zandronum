...
/*
** gl_bonebuffer.cpp
** Buffer data maintenance for bones
**
**/
...
FBoneBuffer::FBoneBuffer(int pipelineNbr):
	mPipelineNbr(pipelineNbr)
{
	int maxNumberOfBones = 256;

	mBufferSize = maxNumberOfBones * ELEMENTS_PER_LIGHT;
	...
	if (screen->IsPoly() || ((screen->hwcaps & RFL_SHADER_STORAGE_BUFFER) && screen->allowSSBO() && !strstr(screen->vendorstring, "Intel")))
	{
		mBufferType = true;
		mBlockAlign = 0;
	...
	{
		mBufferPipeline[n] = screen->CreateDataBuffer(BONES_BINDINGPOINT, mBufferType, false);
...
FBoneBuffer::~FBoneBuffer()
{
	delete mBuffer;
}

void FBoneBuffer::Clear()
{
	mIndex = 0;
...
int FBoneBuffer::UploadBones(FDynLightData &data)
{
